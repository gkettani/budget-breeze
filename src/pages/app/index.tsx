...
export default function App() {
	const { data: transactions, isLoading: isTransactionsLoading } =
		api.transactions.list.useQuery();
	const { data: categories, isLoading: isCategoriesLoading } =
		api.categories.list.useQuery();
	const { data: financialAccounts, isLoading: isFinancialAccountsLoading } =
		api.financialAccounts.list.useQuery();
	...
	const assignedMoney =
		categories?.reduce((acc, category) => acc + category.budget, 0) ?? 0;
	const monthTotalExpenses =
		transactions
			?.filter((t) => t.type === TRANSACTION_TYPE.EXPENSE)
			?.filter(
				(t) =>
					t.date > new Date(new Date().getFullYear(), new Date().getMonth(), 1),
			)
			?.reduce((acc, transaction) => acc + transaction.amount, 0) ?? 0;
	const totalMoney =
		financialAccounts?.reduce((acc, account) => acc + account.balance, 0) ?? 0;

	const expenseProjection =
		categories?.reduce(
			(acc, category) =>
				acc + Math.max(category.target, category.monthExpenseTotal),
			0,
		) ?? 0;
	...
	const metrics = [
		{ title: "Balance", metric: formatCurrency(totalMoney) },
		{
			...
			metric: formatCurrency(monthTotalExpenses),
			projection: formatCurrency(expenseProjection),
		},
						...
						{item.projection && (
							<div className="font-light text-sm text-tremor-content flex items-center gap-2">
								...
								<HoverCard openDelay={400} closeDelay={200}>
									<HoverCardTrigger>
										<Icons.info className="w-4 hover:cursor-pointer hover:text-gray-800" />
									</HoverCardTrigger>
									<HoverCardContent>
										The projected value is calculated based on the targets set
