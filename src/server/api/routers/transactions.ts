	...
	list: protectedProcedure.query(async ({ ctx }) => {
		...
		const transactions = await db
			...
			.leftJoin(
				...
				eq(schema.financialAccounts.id, schema.transactions.financialAccountId),
			)
			...
			const transaction = await db.transaction(async (tx) => {
				...
				if (oldTransaction.categoryId) {
					await tx
						...
						.set({
							budget:
								oldTransaction.type === TRANSACTION_TYPE.EXPENSE
				...
				await tx
					...
					.set({
						balance:
							transaction.type === TRANSACTION_TYPE.EXPENSE
								? increment(
										schema.financialAccounts.balance,
										transaction.amount,
									)
								: decrement(
										schema.financialAccounts.balance,
					...
					.where(
						eq(schema.financialAccounts.id, transaction.financialAccountId),
					);
